...
from nova.utils.formatting import print_error, print_info, print_success
...
def show_config(
    config_file: Path | None = typer.Option(
        None, "--file", "-f", help="Configuration file to show"
    ),
):
    ...
    try:
        ...
        table.add_row(
            "Active Profile", config.active_profile or "None (using direct config)"
        )
...
def list_profiles(
    config_file: Path | None = typer.Option(
        None, "--file", "-f", help="Configuration file to show profiles from"
    ),
):
