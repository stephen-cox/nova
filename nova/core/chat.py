...
from nova.core.ai_client import generate_sync_response, AIError
from nova.models.config import NovaConfig
from nova.models.message import Conversation, MessageRole
from nova.utils.formatting import print_message, print_error, print_success, print_info
...
class ChatSession:
            ...
            try:
                ...
                for filepath, _, _ in history_files:
                    if conversation_id in filepath.stem:
                        self.conversation = self.history_manager.load_conversation(filepath)
                        break
                else:
    ...
    def save_conversation(self, filepath: Optional[Path] = None) -> Path:
    ...
    def get_context_messages(self) -> list:
        ...
        context_data = self.memory_manager.optimize_conversation_context(self.conversation)
...
class ChatManager:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if self.memory_manager.should_summarize_conversation(session.conversation):
                        print_info("💡 Tip: This conversation is getting long. Consider using '/summarize' to create a summary for better memory management.")
    ...
    def _handle_command(self, command: str, session: ChatSession) -> None:
        ...
        if cmd == '/help':
            ...
            try:
                ...
                summary = self.memory_manager.create_conversation_summary(session.conversation)
            ...
            try:
                ...
                print(f"  High importance messages: {stats['high_importance_messages']}")
                print(f"  Recent activity (24h): {stats['recent_activity_24h']}")
                print(f"  Context efficiency: {stats['context_efficiency']:.2%}")
                print(f"  Estimated context tokens: {stats['estimated_context_tokens']}")
                print(f"  Tags: {', '.join(stats['tags']) if stats['tags'] else 'None'}")
                if stats['needs_summarization']:
                    print_info("  💡 This conversation could benefit from summarization (/summarize)")
                if stats['suggested_tags']:
                    print_info(f"  💡 Suggested tags: {', '.join(stats['suggested_tags'])}")
            ...
            # Show suggested tags
            suggested = self.memory_manager.suggest_conversation_tags(session.conversation)
            if suggested:
    ...
    def _generate_ai_response(self, session: ChatSession) -> str:
        ...
        if self.config.ai_model.provider in ["openai", "ollama"]:
            ...
            if session.conversation.summaries:
                system_message += " You have access to conversation summaries to maintain context."
    ...
    def list_conversations(self) -> None:
        ...
        for filepath, title, timestamp in conversations:
            session_id = filepath.stem.split('_', 2)[-1] if '_' in filepath.stem else filepath.stem
