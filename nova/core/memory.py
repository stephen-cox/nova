...
class MemoryManager:
    ...
    def __init__(self, ai_config: AIModelConfig):
        ...
        self.importance_keywords = [
            ...
            "todo", "task", "action", "follow up", "reminder"
        ]
    ...
    def should_summarize_conversation(self, conversation: Conversation) -> bool:
        ...
        if conversation.summaries:
            ...
            messages_since_summary = len(conversation.messages) - last_summary.message_count
    ...
    def create_conversation_summary(self, conversation: Conversation, 
                                  message_count: Optional[int] = None) -> ConversationSummary:
        ...
        if message_count is None:
            ...
            if conversation.summaries:
                last_summary = conversation.summaries[-1]
                messages_to_summarize = conversation.messages[last_summary.message_count:]
            else:
                messages_to_summarize = conversation.messages[:-5]  # Keep recent 5 messages
        else:
    ...
    def _extract_key_topics(self, messages: List[Message]) -> List[str]:
        ...
        for word in words:
            if len(word) > 3 and word not in ['that', 'this', 'with', 'have', 'will', 'from', 'they', 'been', 'their']:
    ...
    def analyze_message_importance(self, message: Message, conversation: Conversation) -> float:
        ...
        if any(indicator in content_lower for indicator in ['```', 'def ', 'class ', 'import ', 'function']):
            importance_score += 0.3
    ...
    def optimize_conversation_context(self, conversation: Conversation, 
        ...
        return {
            ...
            "context_efficiency": len(context_messages) / len(conversation.messages) if conversation.messages else 0
        }
    ...
    def suggest_conversation_tags(self, conversation: Conversation) -> List[str]:
        ...
        for summary in conversation.summaries:
            suggested_tags.update(summary.key_topics[:3])  # Add top 3 topics from each summary
        
        return list(suggested_tags)[:8]  # Limit to 8 suggested tags
    
    def cleanup_old_summaries(self, conversation: Conversation, max_summaries: int = 10) -> None:
