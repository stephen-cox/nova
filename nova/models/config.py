...
class SearchConfig(BaseModel):
    ...
    default_technical_level: str = Field(
        default="intermediate", description="Default technical level for search queries"
    )
    ...
    extraction_backend: str = Field(
        default="yake_only", description="Keyword extraction backend"
    )
    ...
    def validate_enhancement(cls, v: str) -> str:
        ...
        if v not in allowed_modes:
            raise ValueError(
                f"Enhancement mode must be one of: {', '.join(allowed_modes)}"
            )
        return v
    ...
    def validate_technical_level(cls, v: str) -> str:
        ...
        if v not in allowed_levels:
            raise ValueError(
                f"Technical level must be one of: {', '.join(allowed_levels)}"
            )
        return v
    ...
    def validate_timeframe(cls, v: str) -> str:
        ...
        if v not in allowed_timeframes:
            raise ValueError(
                f"Timeframe must be one of: {', '.join(allowed_timeframes)}"
            )
        return v
    ...
    def validate_extraction_backend(cls, v: str) -> str:
        ...
        if v not in allowed_backends:
            raise ValueError(
                f"Extraction backend must be one of: {', '.join(allowed_backends)}"
            )
        return v
