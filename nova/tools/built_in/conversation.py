...
These tools provide conversation history management including listing, searching, saving, and analyzing conversations.
"""
...
async def list_conversations(
    limit: int = 10, include_content: bool = False
) -> list[dict]:
    """
    ...
    Args:
        ...
        include_content: Include conversation metadata (message count, tags, etc.)
...
async def search_conversation_history(
    query: str, limit: int = 5, include_context: bool = True
) -> list[dict]:
    """
    Search through saved conversations for specific content.

    Args:
        query: Search query to find in conversations
        limit: Maximum number of matching conversations to return (1-50)
        include_context: Include snippets of matching message content

    Returns:
        List of matching conversations with relevance information
    """
    # Validate limit
    ...
    try:
        ...
        for filepath, title, timestamp in conversations:
            try:
                ...
                # Search in tags
                tag_match = any(query_lower in tag.lower() for tag in conversation.tags)
...
async def save_conversation(
    ...
    """
    Save the current conversation with optional title and tags.

    Args:
        context: Execution context containing conversation information
        title: Optional title for the conversation
        tags: Optional tags to categorize the conversation
...
async def get_conversation_stats(period_days: int = 30) -> dict:
    """
    Get statistics about conversation history.

    Args:
        period_days: Time period in days to analyze (1-365)
