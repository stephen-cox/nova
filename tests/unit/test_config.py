...
from nova.core.config import ConfigError, ConfigManager
...
class TestConfigManager:
    ...
    def test_save_config(self, temp_dir, sample_config):
        ...
        assert (
            "Set via environment variables" in saved_data["profiles"]["test"]["api_key"]
        )
...
class TestNovaConfig:
    ...
    def test_valid_config_creation(self):
        ...
        test_profile = AIProfile(
            name="test", provider="openai", model_name="gpt-4", api_key="test-key"
        )

        config = NovaConfig(
            profiles={"test": test_profile},
            active_profile="test",
            chat=ChatConfig(history_dir=Path("/tmp/test"), max_history_length=100),
        )
...
class TestAIProfiles:
    ...
    def test_nova_config_with_profiles(self):
        ...
        profile1 = AIProfile(name="gpt4", provider="openai", model_name="gpt-4")
        profile2 = AIProfile(
            name="claude", provider="anthropic", model_name="claude-3-sonnet"
        )

        config = NovaConfig(
            profiles={"gpt4": profile1, "claude": profile2}, active_profile="gpt4"
        )
    ...
    def test_get_active_ai_config_with_profile(self):
        ...
        config = NovaConfig(profiles={"test": profile}, active_profile="test")
    ...
    def test_get_active_ai_config_without_explicit_profile(self):
        ...
        config = NovaConfig(
            profiles={"default": default_profile}, active_profile="default"
        )
    ...
    def test_get_active_ai_config_invalid_profile(self):
        ...
        config = NovaConfig(
            profiles={"default": default_profile}, active_profile="nonexistent"
        )
