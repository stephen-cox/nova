...
from nova.models.message import Conversation, Message, MessageRole, ConversationSummary
...
class TestMemoryManager:
    ...
    def setup_method(self):
        ...
        for i in range(5):
            self.conversation.add_message(
                MessageRole.USER, 
                f"User message {i+1} with some content"
            )
            self.conversation.add_message(
    ...
    def test_analyze_message_importance_keywords(self):
        ...
        score = self.memory_manager.analyze_message_importance(important_msg, self.conversation)
    ...
    def test_analyze_message_importance_question(self):
        ...
        score = self.memory_manager.analyze_message_importance(question_msg, self.conversation)
    ...
    def test_analyze_message_importance_code(self):
        ...
        score = self.memory_manager.analyze_message_importance(code_msg, self.conversation)
    ...
    def test_analyze_message_importance_recent(self):
        ...
        score = self.memory_manager.analyze_message_importance(recent_msg, self.conversation)
    ...
    def test_get_memory_stats(self):
        ...
        required_keys = [
            "message_count", "total_tokens", "duration", "summaries_count",
            "high_importance_messages", "recent_activity_24h", "context_efficiency",
            "estimated_context_tokens", "needs_summarization", "suggested_tags"
        ]
    ...
    def test_extract_key_topics(self):
        ...
        messages = [
            Message(role=MessageRole.USER, content="Let's discuss machine learning algorithms"),
            Message(role=MessageRole.ASSISTANT, content="Neural networks are powerful for pattern recognition"),
            Message(role=MessageRole.USER, content="What about deep learning and artificial intelligence?")
        ]
        ...
        content_words = ["machine", "learning", "neural", "networks", "deep", "artificial", "intelligence"]
...
class TestConversationEnhancements:
    ...
    def test_get_context_for_ai_with_summaries(self):
        ...
        context = self.conversation.get_context_for_ai(token_limit=1000, include_summaries=True)
        
        assert isinstance(context, list)
        # Should include both messages and summary
        has_summary = any("Previous conversation summary" in msg.get("content", "") for msg in context)
        assert has_summary
