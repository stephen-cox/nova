...
from nova.search.models import SearchResult, SearchResponse, SearchError
...
class TestableBaseSearchClient(BaseSearchClient):
    ...
    async def search(self, query: str, max_results: int = 10, **kwargs) -> SearchResponse:
        return SearchResponse(query=query, results=[], total_results=0, search_time_ms=0, provider="Test")
...
class TestBaseSearchClient:
    ...
    async def test_extract_content_with_newspaper(self):
        ...
        with patch('nova.search.engines.base.Article') as mock_article_class:
            ...
            long_content = "This is extracted article content that is sufficiently long to pass the length validation check. " * 3
    ...
    async def test_extract_content_with_readability_fallback(self):
        ...
        with patch('nova.search.engines.base.Article') as mock_article_class, \
            ...
            mock_response.text = "<html><body><div>Original HTML content</div></body></html>"
            mock_response.raise_for_status = MagicMock()
            client.client.get = AsyncMock(return_value=mock_response)
            
            # Make readability succeed
            mock_document = MagicMock()
            mock_document.summary.return_value = "<div>Readability extracted content that is sufficiently long</div>"
            ...
            mock_soup_class.assert_called_once_with("<div>Readability extracted content that is sufficiently long</div>", "html.parser")
