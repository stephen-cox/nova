...
from nova.search.manager import ContentSummarizer, EnhancedSearchManager
...
class TestContentSummarizer:
    ...
    async def test_summarize_content_success(self):
        ...
        mock_ai_client.generate_response.return_value = (
            "This is a focused summary of the content related to the search query."
        )
    ...
    async def test_summarize_content_ai_error(self):
        ...
        content = (
            "Content to summarize. Second sentence. Third sentence. Fourth sentence."
        )
        query = "test query"
    ...
    async def test_synthesize_results(self):
        ...
        mock_ai_client = AsyncMock()
        mock_ai_client.generate_response.return_value = (
            "Synthesized insights from search results."
        )
...
class TestEnhancedSearchManager:
    ...
    def test_initialization_with_ai_client(self):
        ...
        config = {"search": {"enabled": True, "default_enhancement": "fast"}}
        mock_ai_client = MagicMock()
    ...
    def test_build_extraction_config(self):
        ...
        config = {
            "search": {"extraction_backend": "yake_only", "yake_max_keywords": 15}
        }
    ...
    async def test_enhanced_search_with_constraints(self):
        ...
        result = await manager.enhanced_search("test query", constraints=constraints)
    ...
    @pytest.mark.skip(
        reason="Provider fallback on failure not implemented - test needs redesign"
    )
    @pytest.mark.asyncio
    async def test_enhanced_search_provider_fallback(self):
        """Test provider fallback when primary fails"""
        config = {"search": {"enabled": True, "default_provider": "google"}}
    ...
    def test_get_search_client(self):
        ...
        try:
            ...
            raise AssertionError("Should have raised SearchError")
        except SearchError:
            pass  # Expected
